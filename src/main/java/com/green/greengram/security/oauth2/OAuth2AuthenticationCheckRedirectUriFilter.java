...
public class OAuth2AuthenticationCheckRedirectUriFilter extends OncePerRequestFilter {
    ...
    public OAuth2AuthenticationCheckRedirectUriFilter(AppProperties appProperties) {
        ...
        if(requestUri.startsWith(appProperties.getOauth2().getBaseUri())){
            ...
            if(redirectUriParam != null && !hasAuthorizedRedirectUri(redirectUriParam)){ // 허용하지 않은 URI라면
                String errRedirectUrl = UriComponentsBuilder.fromUriString("/err_message")
                        .queryParam("message", "유효한 Redirect URL이 아니다").toUriString();
//                "err_message?message=유효한 Redirect URL이 아닙니다";
    ...
    private boolean hasAuthorizedRedirectUri(String uri) {
        URI ClientRediretUri = URI.create(uri);
        log.info("ClientRediretUri.getHost(): {}", ClientRediretUri.getHost());
        log.info("ClientRediretUri.getPort(): {}", ClientRediretUri.getPort());

        for(String redirectUri : appProperties.getOauth2().getAuthorizedRedirectUris()) {
            URI authorizedUri = URI.create(redirectUri);
            if(ClientRediretUri.getHost().equalsIgnoreCase(authorizedUri.getHost())
                    && ClientRediretUri.getPort() == authorizedUri.getPort()
                    &&ClientRediretUri .getPath().equals(authorizedUri.getPath())) {
                return true;
            }
        }
        return false;
